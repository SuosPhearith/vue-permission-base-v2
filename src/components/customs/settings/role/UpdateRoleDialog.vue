<template>
  <VDialog v-model="isDialogVisible" persistent class="v-dialog-lg">
    <DialogCloseBtn
      ...
      v-if="!pending"
    />
    <VCard title="Update Role" :loading="loading">
      <VCardText>
...
const setup = async () => {
  ...
  try {
    // Fetch role details
    const resUser = await axiosInstance.get(`/setting/role/${props.initId}`);
    name.value = resUser.data.data.name;
    selectedPermissions.value = resUser.data.data.permission_ids;

    // Fetch available permissions
    const resPermissions = await axiosInstance.get("/setting/module");
...
const handleUpdate = async () => {
  ...
  if (isValid?.valid) {
    ...
    try {
      ...
      await axiosInstance.put(`/setting/role/${props.initId}`, roleData);
